      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, flexWrap: "wrap", gap: 12 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <span style={{ ...dm("13px"), fontWeight: 500, color: "var(--text-primary)", minWidth: 170, textAlign: "center" }}>{headerLabel}</span>
            <button className="tbtn" onClick={() => nav(1)}>›</button>
          </div>
      ...
      <div style={{ flex: 1, overflow: "auto", borderRadius: 16, border: "1px solid var(--border-light)", background: "var(--bg-card)", boxShadow: "0 1px 3px rgba(28,25,23,0.06)" }} className="fi">
          ...
          <div style={{ display: "flex", flexDirection: "column", minWidth: 700 }}>
            <div style={{ display: "flex", borderBottom: "2px solid var(--border-light)", background: "var(--bg-card)", position: "sticky", top: 0, zIndex: 8 }}>
              ...
              {weekDates.map((d, i) => {
                ...
                return (
                  <div key={i} style={{ flex: 1, padding: "10px 8px", textAlign: "center", borderLeft: "1px solid var(--border-light)" }}>
                    <div style={{ ...dm("11px"), color: "var(--text-muted)", marginBottom: 4 }}>{DAY_SHORT[i]}</div>
                    <div style={{ width: 32, height: 32, borderRadius: "50%", background: isToday ? "var(--accent)" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 4px" }}>
                      <span style={{ fontFamily: "var(--font-lora)", fontSize: 15, fontWeight: 600, color: isToday ? "#FAF7F2" : "var(--text-primary)" }}>{d.getDate()}</span>
                    </div>
                    {cnt > 0 && <div style={{ ...dm("10px"), color: "var(--text-muted)" }}>{cnt} cita{cnt > 1 ? "s" : ""}</div>}
                  </div>
                );
              })}
            </div>
            <div style={{ display: "flex" }}>
              <div style={{ width: 60, flexShrink: 0 }}>
                {HOURS.map((h, i) => (
                  <div key={i} style={{ height: HOUR_H, display: "flex", alignItems: "flex-start", justifyContent: "flex-end", paddingRight: 10, paddingTop: 4 }}>
                    <span style={{ ...dm("11px"), color: "var(--text-muted)" }}>{h}</span>
                  </div>
                ))}
              </div>
        ...
        {view === "day" && (
          <div style={{ display: "flex", height: "100%", minHeight: 600 }}>
            <div style={{ width: 60, flexShrink: 0 }}>
              {HOURS.map((h, i) => <div key={i} style={{ height: HOUR_H, display: "flex", alignItems: "flex-start", justifyContent: "flex-end", paddingRight: 10, paddingTop: 4 }}><span style={{ ...dm("11px"), color: "var(--text-muted)" }}>{h}</span></div>)}
            </div>
            ...
            {/* Sidebar lista */}
            <div style={{ width: 260, borderLeft: "1px solid var(--border-light)", padding: 16, overflowY: "auto", flexShrink: 0 }}>
        ...
        {view === "month" && (
          <div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", borderBottom: "2px solid var(--border-light)", position: "sticky", top: 0, background: "var(--bg-card)", zIndex: 8 }}>
              {DAY_SHORT.map((d, i) => <div key={i} style={{ padding: "10px 12px", ...dm("12px"), fontWeight: 500, color: "var(--text-muted)", textAlign: "center", borderRight: i < 6 ? "1px solid var(--border-light)" : "none" }}>{d}</div>)}
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)" }}>
              {monthCells.map((d, i) => {
                if (!d) return <div key={i} style={{ minHeight: 90, borderRight: "1px solid var(--border-light)", borderBottom: "1px solid var(--border-light)", background: "var(--surface)", opacity: .25 }} />;
                const iso = toISO(d);
                const isToday = iso === TODAY;
                const dayAppts = appts.filter(a => a.date === iso);
                return (
                  <div key={i} className={`mcell${isToday ? " tc" : ""}`} onClick={() => { setDate(d); setView("day"); }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                      <span style={{ width: 26, height: 26, borderRadius: "50%", background: isToday ? "var(--accent)" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "var(--font-lora)", fontSize: 13, fontWeight: isToday ? 600 : 400, color: isToday ? "#FAF7F2" : "var(--text-primary)" }}>{d.getDate()}</span>
                      {dayAppts.length > 0 && <span style={{ ...dm("10px"), color: "var(--text-muted)" }}>{dayAppts.length}</span>}
                    </div>
                    <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                      {dayAppts.slice(0, 3).map((a, ai) => (
                        <div key={ai} onClick={e => { e.stopPropagation(); setSelected(a); }}
                          style={{ padding: "2px 6px", borderRadius: 4, background: `${a.color}18`, borderLeft: `2px solid ${a.color}`, ...dm("10px"), color: a.color, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", cursor: "pointer" }}>
                          {a.startTime} {a.patientName.split(" ")[0]}
                        </div>
                      ))}
                      {dayAppts.length > 3 && <div style={{ ...dm("10px"), color: "var(--text-muted)", paddingLeft: 6 }}>+{dayAppts.length - 3} más</div>}
                    </div>
                  </div>
                );
              })}
