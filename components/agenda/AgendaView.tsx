...
// ─── Helpers de fecha SIEMPRE en zona Venezuela ───────────────────────────────
// "YYYY-MM-DD" en hora Venezuela
function toISO(d: Date): string {
  return d.toLocaleDateString("en-CA", { timeZone: TZ });
}
// "HH:MM" (24h) de un ISO timestamp en hora Venezuela — usado para cálculos de posición
function toHHMM(iso: string): string {
  return new Date(iso).toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit", timeZone: TZ });
}
// Minutos desde medianoche a partir de "HH:MM" — solo recibe formato 24h limpio
...
// Hora formateada para mostrar al usuario
function fmtTime(iso: string): string {
  return new Date(iso).toLocaleTimeString("es-VE", { hour: "2-digit", minute: "2-digit", timeZone: TZ });
}
function fmtDateLabel(iso: string): string {
  return new Date(iso).toLocaleDateString("es-VE", { weekday: "long", day: "numeric", month: "long", timeZone: TZ });
}
...
function AppointmentForm({ initial, patients, onSave, onClose, defaultDate }: {
  initial?: Appointment | null; patients: PatientOption[];
  onSave: (data: any) => Promise<void>; onClose: () => void; defaultDate?: string;
}) {
  ...
  const [startTime, setStartTime] = useState(initial ? toHHMM(initial.start_time) : "09:00");
  const [endTime, setEndTime]     = useState(initial ? toHHMM(initial.end_time) : "09:50");
      ...
      <div style={{ position:"fixed", top:"50%", left:"50%", transform:"translate(-50%,-50%)", background:"var(--bg-card)", borderRadius:20, padding:28, width:"min(480px,94vw)", maxHeight:"90vh", overflowY:"auto", zIndex:71, boxShadow:"0 8px 32px rgba(28,25,23,0.18)", animation:"popIn .2s ease" }}>
        ...
        <div style={{ marginBottom:20 }}>
          ...
          <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={3}
            placeholder="Ej: Recordar revisar tarea, traer resultados de examen..."
      ...
      <div style={{ position:"fixed", top:"50%", left:"50%", transform:"translate(-50%,-50%)", background:"var(--bg-card)", borderRadius:20, padding:28, width:"min(400px,92vw)", zIndex:81, boxShadow:"0 8px 28px rgba(28,25,23,0.18)", animation:"popIn .2s ease" }}>
        ...
        <div style={{ display:"flex", gap:10 }}>
          ...
          <button onClick={() => onConfirm(reason)}
            style={{ flex:1, padding:"10px", borderRadius:10, border:"none", background:"var(--red)", color:"#fff", fontFamily:"var(--font-dm-sans)", fontSize:13, fontWeight:600, cursor:"pointer" }}>
            Cancelar cita
          </button>
        </div>
      </div>
...
function DetailModal({ appt, onClose, onEdit, onCancel, onMarkDone, onMarkNoShow, onReschedule }: {
  appt: Appointment; onClose: () => void; onEdit: () => void; onCancel: () => void;
  onMarkDone: () => void; onMarkNoShow: () => void; onReschedule: () => void;
}) {
...
export function AgendaView() {
  ...
  const nowVE        = new Date().toLocaleTimeString("en-GB", { hour:"2-digit", minute:"2-digit", timeZone: TZ });
  const todayLineTop = ((toMin(nowVE) - toMin("08:00")) / 60) * HOUR_H;
  ...
  async function handleCreate(data: any) {
    setSaving(true);
    const res = await fetch("/api/appointments", { method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify({ ...data, psychologistId: userId }) });
    const json = await res.json();
    if (!res.ok) throw new Error(json.error ?? "Error al crear");
    await load(); setFormAppt(null); setSaving(false);
  }

  async function handleEdit(data: any) {
    const appt = formAppt !== "new" ? formAppt as Appointment : null;
    if (!appt) return;
    setSaving(true);
    const res = await fetch("/api/appointments", { method:"PUT", headers:{"Content-Type":"application/json"}, body: JSON.stringify({ ...data, id: appt.id, psychologistId: userId }) });
    const json = await res.json();
    if (!res.ok) throw new Error(json.error ?? "Error al editar");
    await load(); setFormAppt(null); setDetailAppt(null); setSaving(false);
  }
  ...
  async function handleStatusChange(appt: Appointment, status: AppStatus, cancelReason?: string) {
    ...
    await fetch("/api/appointments", {
      ...
      body: JSON.stringify({ id: appt.id, psychologistId: userId, status, patientName: patient ? `${patient.first_name} ${patient.last_name}` : appt.title, patientEmail: patient?.email ?? null, start_time: appt.start_time, end_time: appt.end_time, modality: appt.modality, notes: appt.notes, type: appt.type, cancel_reason: cancelReason ?? null }),
    });
    await load(); setDetailAppt(null); setCancelAppt(null); setSaving(false);
  }
          ...
          <div style={{ display:"flex", flexDirection:"column", minWidth:600 }}>
            <div style={{ display:"flex", borderBottom:"2px solid var(--border-light)", background:"var(--bg-card)", position:"sticky", top:0, zIndex:8 }}>
              ...
              {weekDates.map((d, i) => {
                ...
                return (
                  <div key={i} style={{ flex:1, padding:"8px 4px", textAlign:"center", borderLeft:"1px solid var(--border-light)", cursor:"pointer" }}
        ...
        {view === "day" && (
          <div style={{ display:"flex", height:"100%", minHeight:600 }}>
            ...
            <div style={{ flex:1, borderLeft:"1px solid var(--border-light)", position:"relative" }}>
              ...
              {appts.filter(a => toISO(new Date(a.start_time)) === toISO(date)).map(a => (
                <ApptBlock key={a.id} appt={a} onClick={() => setDetailAppt(a)} />
              ))}
            ...
            <div className="agenda-day-sidebar" style={{ width:240, borderLeft:"1px solid var(--border-light)", padding:14, overflowY:"auto", flexShrink:0, flexDirection:"column" }}>
              ...
              {appts
                .filter(a => toISO(new Date(a.start_time)) === toISO(date))
                .sort((a,b) => a.start_time.localeCompare(b.start_time))
                .map(appt => {
                  const pName = appt.patient ? `${appt.patient.first_name} ${appt.patient.last_name}` : appt.title;
                  const color = appt.patient ? colorForName(pName) : "#8B7355";
                  return (
                    ...
                    >
                      <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                        ...
                        <span style={{ ...dm("13px"), fontWeight:500, color:"var(--text-primary)", flex:1, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{pName}</span>
        ...
        {view === "month" && (
          <div>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", borderBottom:"2px solid var(--border-light)", position:"sticky", top:0, background:"var(--bg-card)", zIndex:8 }}>
              {DAY_SHORT.map((d,i) => (
                <div key={i} style={{ padding:"8px 4px", ...dm("11px"), fontWeight:500, color:"var(--text-muted)", textAlign:"center", borderRight:i<6?"1px solid var(--border-light)":"none" }}>{d}</div>
              ))}
            </div>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)" }}>
              {monthCells.map((d, i) => {
                ...
                const dayAppts = appts.filter(a => toISO(new Date(a.start_time)) === iso);
                return (
                  <div key={i} className={`mcell${isToday?" tc":""}`} style={{ minHeight:80, padding:6 }}
                    onClick={() => { setDate(d); setView("day"); }}>
