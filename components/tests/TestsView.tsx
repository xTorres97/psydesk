...
export function TestsView() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", maxWidth: 1280, margin: "0 auto" }}>
      ...
      {/* Tabs */}
      <div style={{ display: "flex", borderBottom: "1px solid var(--border-light)", marginBottom: 20, flexShrink: 0 }}>
        {(["biblioteca", "enviados", "resultados"] as TabSection[]).map(t => (
          <button
            ...
            style={{
              ...
              cursor: "pointer", transition: "all .15s", ...dm("13px"),
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden", gap: 16 }}>
          ...
          {tab === "biblioteca" && (
            <>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 20 }}>
                {CATEGORIES.map(c => (
                  <span key={c} className={`chip${filterCat === c ? " on" : ""}`} onClick={() => setFilterCat(c)} style={{ cursor: "pointer" }}>{c}</span>
                ))}
              </div>
                ...
                {filteredTests.map(t => (
                  <div
                    ...
                    style={{
                      ...
                      borderColor: selectedTest?.id === t.id ? "var(--accent-light)" : "var(--border-light)",
                      background: selectedTest?.id === t.id ? "var(--accent-bg)" : "var(--bg-card)",
                      transition: "all .2s",
                    }}
        ...
        {selectedTest && tab === "biblioteca" && (
          <div style={{ width: 280, flexShrink: 0, borderRadius: 16, overflow: "hidden", border: "1px solid var(--border-light)", background: "var(--bg-card)", padding: 20, overflowY: "auto", animation: "slideIn .25s ease" }}>
            ...
            {selectedTest.questions > 0 && (
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
                {[
                  { label: "Ítems",      val: selectedTest.questions },
                  { label: "Duración",   val: `~${selectedTest.duration} min` },
                  { label: "Categoría",  val: selectedTest.category },
                  { label: "Validación", val: selectedTest.validated ? "Clínica" : "—" },
                ].map((r, i) => (
            ...
            <button
              className="btn-p"
              style={{ width: "100%" }}
              onClick={() => { setSendModal(selectedTest); setSelectedTest(null); }}
            >
              📤 Enviar a paciente
            </button>
          </div>
      ...
      {sendModal && (
        <div
          style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50, backdropFilter: "blur(4px)" }}
          onClick={() => setSendModal(null)}
        >
          <div
            style={{ background: "var(--bg-card)", borderRadius: 20, padding: 28, width: 420, maxWidth: "90vw", boxShadow: "0 8px 28px rgba(28,25,23,0.14)", animation: "popIn .2s ease" }}
